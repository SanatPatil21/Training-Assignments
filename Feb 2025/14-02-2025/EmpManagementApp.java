...
class DatabaseConnection {
     ...
     */
    static String url = System.getenv("DB_URL"); // Read from environment variables
...
class DatabaseOperations {
    ...
    // to this method, order by ka param pass karna hai
    ...
    public static void appraisal(int eid, double amountToBeHiked) {
        String query = "UPDATE employees SET salary = salary + ? where eid = ?";
        try (
                Connection con = DatabaseConnection.getConnection();
                PreparedStatement stmt = con.prepareStatement(query);) {
            stmt.setDouble(1, amountToBeHiked);
            stmt.setInt(2, eid);
            boolean succ = stmt.execute();
            if (succ) {
                System.out.println("Employee Added Successfully");
    ...
    public static boolean searchEmployeeById(int eid) {
        ...
        try (
            ...
            stmt.setInt(1, eid);
            ResultSet rs = stmt.executeQuery();
            if (rs != null && rs.next()) { // Check if there is a valid result
    ...
    public static int validateChoice(int max, BufferedReader br) {
        ...
        while (true) {
            ...
            try {
                ...
                //Just to break the loop in docker
                return -1;
            }
        }
